...
#[expect(
    clippy::unwrap_used,
    reason = "Test panic-on-setup-failure is acceptable; benchmark not in hot path."
)]
fn measure_one(queue: &std::path::PathBuf, n: usize) -> Duration {
    let start = Instant::now();
    let _ = run_hook(queue, &format!("sess-{n}"), "/tmp/raw.jsonl");
    start.elapsed()
}
